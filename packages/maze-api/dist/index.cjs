...
const e = require("effect"),
	y = require("node:sqlite"),
	A = require("@fastify/cors"),
	L = require("fastify"),
...
CREATE TABLE IF NOT EXISTS players (
	...
	O = "SELECT * FROM players WHERE playerID = ?",
...
const d = e.Schema.Struct({
	...
	g = (t) => {
		...
		return {
			get: (c, r) =>
				e.Effect.tryPromise({
					try: () => Promise.resolve(a.prepare(c).get(...r)),
					catch: (s) => s,
				}),
			all: (c) =>
				e.Effect.tryPromise({
					try: () => Promise.resolve(a.prepare(c).all()),
					catch: (r) => r,
				}),
			run: (c, r) =>
				e.Effect.tryPromise({
					try: () => Promise.resolve(a.prepare(c).run(...(r ?? []))),
					catch: (s) => s,
				}),
		};
	},
	o = e.Effect.Service()("DatabaseService", {
		effect: e.Effect.succeed(g("data.sqlite")),
	}),
	q = e.Layer.succeed(o, g(":memory:")),
	R = (t) => ({
		initMazeSchema: e.pipe(
			t.run(D),
			e.Effect.map(() => t),
		),
		insertMaze: (a) =>
	...
	n = e.Effect.Service()("MazeDBService", {
		dependencies: [o.Default],
		effect: o.pipe(e.Effect.map(R)),
	});
e.Effect.Service()("MazeDBService", {
	dependencies: [q],
	effect: o.pipe(e.Effect.map(R)),
});
const V = (t) => ({
		...
		getAllPlayers: e.pipe(
			t.all(C),
			e.Effect.map((a) => e.Schema.decodeUnknownSync(k)(a)),
			e.Effect.catchTag("DatabaseError", (a) => e.Effect.fail(a)),
		),
		deletePlayer: (a) =>
			e.pipe(
				...
				t.get("/", async (a, c) => {
					...
					t.get(F, async (a, c) => {
						...
						return i
							...
							.catch((s) => {
								console.error("Error fetching maze:", s),
					...
					t.get(Y, async (a, c) =>
						i
							...
							.catch(() => {
								c.status(l.INTERNAL_SERVER_ERROR).send({
									status: "error",
									error: "Failed to fetch maze metada",
								});
							}),
					),
					t.get(G, async (a, c) =>
						i
							...
							.catch(() => {
								c.status(l.INTERNAL_SERVER_ERROR).send({
									status: "error",
									error: "Failed to fetch all mazes",
								});
							}),
					);
			}),
	...
	i = {
		getMaze: (t) =>
			e.pipe(
				n,
				e.Effect.flatMap((a) => a.getMazeById(t)),
		...
		getAllMetaData: () =>
			e.pipe(
				n,
				e.Effect.flatMap((t) => t.getMetadata),
				e.Effect.provide(n.Default),
				e.Effect.runPromise,
			),
		getAllMazes: () =>
			e.pipe(
				n,
				e.Effect.flatMap((t) => t.getAllMazes),
				e.Effect.provide(n.Default),
				e.Effect.runPromise,
			),
	},
	...
	p = {
		getPlayer: (t) =>
			e.pipe(
				f,
				e.Effect.flatMap((a) => a.getPlayerById(t)),
				e.Effect.provide(f.Default),
				e.Effect.runPromise,
			),
		getAllPlayers: () =>
			e.pipe(
				f,
				e.Effect.flatMap((t) => t.getAllPlayers),
				e.Effect.provide(f.Default),
				e.Effect.runPromise,
			),
	},
	J = () => {
		...
		return (
			t.register(A, { origin: "*" }),
			{
				start: () =>
					e.Effect.async((a) => {
						const c = Number(process.env.API_PORT || "8080");
						t.listen({ host: "0.0.0.0", port: c }, (r, s) => {
							r && (console.error("Server error:", r), process.exit(1)),
				...
				register: (a) =>
					e.pipe(
						t,
						a,
						e.Effect.tap(() => e.Effect.log("Routes registered successfully")),
					),
				process: () => {
					e.Effect.sync(() => {
						process.on("SIGINT", () => {
