...
import { makeDB, MazeDB, MazeDBLive } from "../db/Database.js";
import {
  CREATE_MAZE_TABLE,
  INSERT_MAZE,
  SELECT_ALL_MAZE,
...
// Tag for DI
export const MazeDBService = Context.Tag("MazeDBService")<MazeDBService, MazeDBService>();
...
export const MazeServiceLayer = Layer.effect(
  ...
  pipe(
    MazeDB,
    Effect.map(MazeServiceImpl)
  )
);

export const LayerMazeDB = Layer.merge(
  MazeDBLive,
  MazeServiceLayer
)
