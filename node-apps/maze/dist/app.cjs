...
const G = `http://${process.env.HOST}:${Number(process.env.PORT || "8080")}`,
	m = {
		right: { x: 0, y: 1 },
		down: { x: 1, y: 0 },
		left: { x: 0, y: -1 },
...
class l extends e.Context.Tag("BuilderMaze")() {
	static Live = l.of(W);
}
class n {
	constructor(t) {
...
const Y = e.Schema.Boolean,
	v = e.Schema.Array(Y),
	q = e.Schema.Struct({ vertical: v, horizontal: v }),
	w = e.Schema.Struct({
	...
	B = e.Schema.transform(X, E, {
		encode: (c) => ({ ...c, grid: JSON.stringify(c.grid) }),
		decode: (c) => ({ ...c, grid: JSON.parse(c.grid) }),
	});
e.Schema.Array(B);
const R = e.Schema.Struct({ x: e.Schema.Number, y: e.Schema.Number });
e.Schema.Struct({ maze: E, currentPosition: R, playerMoves: R });
const V = e.Schema.Struct({
	...
	pe = ({ visited: c, maze: t, stack: a }) =>
		e.pipe(
			...
			e.Effect.flatMap((f) => {
				...
				const { x: r, y: s, path: p } = f;
				if (r === t.numRows - 1 && s === t.numCols - 1)
					return e.Effect.succeed(p);
				const z = `${r},${s}`;
				return c.has(z)
					? e.Effect.succeed([])
					: (c.add(z),
						e.pipe(
							...
							e.Effect.map((D) => {
								D.forEach((h) => {
									a.push({ x: h.x, y: h.y, path: [...p, ...h.path] });
								});
							}),
						));
			}),
	...
	de = (c, t, a) =>
		e.pipe(
			...
			e.Effect.flatMap((f) =>
				f.get(
					Object.entries(t).reduce((r, [s, p]) => r.replace(`:${s}`, p), c),
				),
			),
...
class g extends e.Effect.Service()("Maze", {
	...
	effect: e.pipe(
		...
		e.Effect.map(({ player: t, maze: a, gameMode: f }) =>
			e.Schema.decodeUnknownSync(J)({ player: t, maze: a, gameMode: f }),
		),
		e.Effect.tap((t) =>
			e.pipe(
				i,
...
e.Layer.mergeAll(g.Default, e.Layer.effect(i, e.Ref.make(P)));
const ze = ({ x: c, y: t }, a) => (c === a ? t : -1),
	ve = ({ vertical: c, horizontal: t }, a) =>
		e.pipe(
			e.Effect.all({ builderMaze: l, currentPosition: d, mazeData: i }),
	...
	be = e.pipe(
		e.Effect.all({ mazeData: i, builder: l }),
		...
		e.Effect.provideService(l, l.Live),
	),
	...
	M = g.pipe(
		...
		e.Effect.provideServiceEffect(i, e.Ref.make(P)),
	);
