...
// import * as t from "@nadir/starlight/styles/global.css"
...
function generateMaze(rows: number, cols: number) {
	...
	while (stack.length) {
		...
		const neighbors = directions
			...
			.filter(
				({ nx, ny }) =>
					nx >= 0 && ny >= 0 && nx < rows && ny < cols && !visited[nx][ny],
			);
		...
		if (dx === 1)
			(grid[x].horizontal[y] = true), (grid[nx - 1].horizontal[y] = true);
		if (dx === -1)
			(grid[nx].horizontal[ny] = true), (grid[x - 1].horizontal[ny] = true);
		if (dy === 1)
			(grid[x].vertical[y] = true), (grid[x].vertical[y - 1] = true);
		if (dy === -1)
			(grid[nx].vertical[ny] = true), (grid[nx].vertical[ny - 1] = true);
		visited[nx][ny] = true;
