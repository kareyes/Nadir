...
const getMazeCellProps = (row: number, col: number) => ({
	...
	hasTopWall: row === 0 || (row > 0 && maze.grid[row - 1].horizontal[col] === false),
	hasLeftWall: col === 0 || (col > 0 && maze.grid[row].vertical[col - 1] === false),
	hasBottomWall: row === maze.numRows - 1 || maze.grid[row].horizontal[col] === false,
	hasRightWall: col === maze.numCols - 1 || maze.grid[row].vertical[col] === false,
	isInSolution: Array.isArray(solutionPath) &&
